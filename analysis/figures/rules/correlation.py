...
pax1 = plt.subplot2grid((1,4), (0,0), colspan=2,projection="polar",frameon=False)
pax2 = plt.subplot2grid((1,4), (0,2), projection="polar",frameon=False)
pax3 = plt.subplot2grid((1,4), (0,3))
...
correlations[correlations[:,1]<0,1] = correlations[correlations[:,1]<0,1] + 2 *pi

hista2=binned_statistic_2d(x=correlations[:,0],y=correlations[:,1],values=correlations[:,2], statistic='mean', bins=[r2,theta2])[0]  
...
pax3.plot(xp, gpdf,'-',linewidth=3,color="slategrey")
#pax3.fill_between(xp,np.zeros_like(xp), gpdf,alpha=0.4)#, fc='#AAAAFF')
pax3.hist(moves,normed=True,range=[-xs,xs],bins=40,color="midnightblue")
pax3.set_xlim(-xs,xs)


#pax3.plot(np.arange(0,1,0.1),np.sin(np.arange(0,1,0.1)))
#pax3.hist(moves,range=[0.1,0.1],bins=100)
pax3.text(-0.2, 1.1, 'D', transform=pax3.transAxes, fontsize=24, va='top', ha='right')
pax3.set_xlabel('Turn angle',fontsize=16)
pax3.set_ylabel('Probability density',fontsize=16)

#plt.show()
plt.savefig("interactions.png",bbox_inches='tight',dpi=300)
